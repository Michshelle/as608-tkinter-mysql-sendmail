...
import mysql_query as mysql_query
import datetime
...
class Operation:
	...
	def finger_search(self):
		...
		print([f"{x:02x}" for x in r])
		return r[0]

	# 这个自己写的为了用其他的返回数据做分析。
	def finger_search_all_return(self):
		"""Asks the sensor to search for a matching fingerprint starting at
		slot 1. Stores the location and confidence in self.finger_id
		and self.confidence. Returns the packet error code or OK success"""
		self.read_sysparam()
...
def enroll_finger_to_device(session, as608_lib):
	...
	#get the location number that from low to high in order and no skipping
...
# 这个自己写的为了用其他的返回数据做分析。
def search_fingerprint_on_dev(session, as608_lib, is_sign_in):
	"""Get a finger print nd see if it matches!"""
	try:
	...
	if session.image_2_tz(1) != as608_lib.OK:
		return False
	time.sleep(0.5)
	result = session.finger_search_all_return()
	if isinstance(result, list):
		if result[0] == '00':
			#确定是迁入还是迁出。
			if is_sign_in == "signin":
				caozuo = "签到"
				result.append('IN')
			else:
				caozuo = "离开"
			...
			result.append(now.strftime('%Y-%m-%d'))  # 转为字符串，可直接插入 MySQL 的 DATE 字段
			result.append(now.strftime('%H:%M:%S'))  # 转为字符串，可直接插入 MySQL 的 TIME 字段
			# 查询用户信息
			user_info = mysql_query.query_by_user_result(result)
			if user_info:
				print(f"{caozuo}成功,姓名:{user_info[0][0]}，时间：{now.strftime('%Y-%m-%d %H:%M:%S')}")
			return True
		elif result[0] == '09':
			print(f"没有找到匹配的指纹")
			return False
...
def empty_ram_library(session,as608_lib):
	"""Empty all fingerprint templates stored in the device"""
	print("Emptying library...", end="", flush=True)
	i = session.empty_library()
	if i == as608_lib.OK:
		print("Emptied")
	else:
		if i == as608_lib.DBCLEARFAIL:
...
def enroll_to_file_and_device(session, as608_lib, save_location, file_name):
	"""Take a 2 finger images and template it, then store it both in a file and device"""
	saved_directory_file_name = save_location+"/"+file_name+".dat"
